...
public class Main {
    ...
    private static String input = "bdwdjjo avricm cjbmj ran lmfsom ivsof\n" +
    ...
    public static void main(String[] args) {
        System.out.println(uniquePassphrases());
    }

    private static List<List<String>> toStringListList() {
        return asList(input.split("\n")).stream()
                .map(string -> asList(string.split(" ")))
                .collect(Collectors.toList());
    }

    private static Integer uniquePassphrases(){
        List<List<String>> input = toStringListList();
        return input.stream()
                .filter(uniqueWords())
                .mapToInt(list -> 1)
                .sum();
